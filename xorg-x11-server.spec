...
# 638234 - Bump classic driver default resolution to 1024x768 (Z - 668514)
...
%endif
%if 0%{?rhel}
%define bodhi_flags --with-vendor-web="https://www.redhat.com/apps/support/" --with-vendor-name="Red Hat, Inc."
%endif

# --with-pie ?
autoreconf -v --install || exit 1
export CFLAGS="${RPM_OPT_FLAGS} -rdynamic $CFLAGS -Wl,-z,relro"
%configure --enable-maintainer-mode %{xservers} \
	...
	--enable-xselinux --enable-record --without-dtrace \
...
%changelog
* Thu Feb 24 2011 Dave Airlie <airlied@redhat.com> 1.7.7-26.3
- Fix two issues reported in TPS runs in spec file (#668514)

* Wed Feb 02 2011 Dave Airlie <airlied@redhat.com> 1.7.7-26.2
- xserver-1.7.7-classic-default-mode.patch: Bump classic driver default mode to 1024x768 (#668514)

* Wed Aug 11 2010 Adam Jackson <ajax@redhat.com> 1.7.7-26
- xserver-1.7.7-randr-cursor-dead-zones.patch: Fix for rotation (#585283)

* Mon Aug 09 2010 Adam Jackson <ajax@redhat.com> 1.7.7-25
- xserver-1.7.7-ddc-probe-less.patch: Don't probe for DDCCI or EEPROM (#620333)

* Fri Aug 06 2010 Adam Jackson <ajax@redhat.com> 1.7.7-24
- xserver-1.7.7-randr-cursor-dead-zones.patch: Clip cursor motion to CRTC
  boundaries if all your CRTCs are contiguous. (#585283)

* Fri Jul 30 2010 Adam Jackson <ajax@redhat.com> 1.7.7-23
- xserver-1.7.7-lid-hack.patch: Account for lid status when multiple outputs
  appear to be connected. (#601319)
- xserver-1.6.99-randr-error-debugging.patch: Fix thinko to print the
  contents of the triggering request, not the address.

* Wed Jul 28 2010 Peter Hutterer <peter.hutterer@redhat.com> 1.7.7-22
- xserver-1.7.7-postfix-DCE-PointerKeys.patch: post-fix event sequences by
  PointerKeys with DeviceChangeEvents (#618422)
- xserver-1.7.7-reset-unused-classes.patch: reset the classes after copying
  them from the buffer. Fixes double-free on CloseDownDevices() exposed
  by xserver-1.7.7-postfix-DCE-PointerKeys.patch.

* Tue Jul 13 2010 Adam Jackson <ajax@redhat.com> 1.7.7-21
- xserver-1.7.7-int10-reserved-areas.patch: Fix vesa driver (and int10
  services generally) when VBIOS code exists below the VGA memory
  hole. (#605302)

* Mon Jul 12 2010 Adam Jackson <ajax@redhat.com> 1.7.7-20
- xserver-1.7.7-xephyr-24bpp.patch: Fix crash in Xephyr when running on a
  24bpp host framebuffer. (#581505)

* Fri Jul 09 2010 Peter Hutterer <peter.hutterer@redhat.com> 1.7.7-19
- xserver-1.7.7-release-xtest-on-phys-buttons.patch: release XTEST buttons
  on physical button releases (#607150)

* Wed Jul 07 2010 Adam Jackson <ajax@redhat.com> 1.7.7-18
- xserver-1.7.6-default-modes.patch: Add xf86DefaultModes alias (#611612)

* Wed Jun 30 2010 Peter Hutterer <peter.hutterer@redhat.com> 1.7.7-17
- xserver-1.7.7-xkb-purge-includes.patch: purge some includes, from master,
  prep-work for lockedPtrBtns.
- xserver-1.7.7-xkb-rename-fakebutton.patch: rename + move XKB fake PointerKey
  event functions, prep-work for lockedPtrBtns.
- xserver-1.7.7-xkb-pointerkeys-on-master.patch: don't send key and motion
  events for PointerKeys motion events. (related #607150)
- xserver-1.7.7-xkb-lockedPtrBtns-state-merge.patch: merge the state from
  SD's lockedPtrBtns to the MD to avoid stuck buttons (#607150)

* Mon Jun 28 2010 Peter Hutterer <peter.hutterer@redhat.com> 1.7.7-16
- xserver-1.7.7-dga-master-keyboard.patch: avoid DGA segfault with floating
  slaves (#607045)
- xserver-1.7.7-event-mask-revert.patch: stuck passive grab when ButtonPress
  is not in the event mask (#607410)
- xserver-1.7.7-sd-keyboard-controls.patch: typo fix, apply bell settings to all
  keyboards (#607051)
- xserver-1.7.7-showopts-segv.patch: segfault during Xorg -showopts (#607022) 

* Wed Jun 23 2010 Adam Jackson <ajax@redhat.com> 1.7.7-15
- xserver-1.7.7-improve-mode-selection.patch: Fix aspect match for classic
  drivers (#604057)

* Wed Jun 23 2010 Dave Airlie <airlied@redhat.com> 1.7.7-14
- make ephyr resizeable (#600505)

* Wed Jun 23 2010 Dave Airlie <airlied@redhat.com> 1.7.7-13
- fix overlapping pixel with rotated screen (#574486)

* Mon Jun 21 2010 Dave Airlie <airlied@redhat.com> 1.7.7-12
- regression in composite resize code found - backport fix from upstream (#586567)

* Wed Jun 16 2010 Peter Hutterer <peter.hutterer@redhat.com> 1.7.7-11
- xserver-1.7.7-xkb-geom-copy.patch: copy omitted XKB geom sections (#588640)

* Tue Jun 15 2010 Adam Jackson <ajax@redhat.com> 1.7.7-10
- Add xserver to console.apps (#602187)

* Fri Jun 11 2010 Peter Hutterer <peter.hutterer@redhat.com> 1.7.7-9
- xserver-1.7.7-modifier-keycount.patch: Don't copy the modifier keycount
  when copying device classes (#602511)

* Thu Jun 10 2010 Dave Airlie <airlied@redhat.com> 1.7.7-8
- fix up composite patch to apply in correct place (#586567)

* Thu Jun 10 2010 Peter Hutterer <peter.hutterer@redhat.com> 1.7.7-7
- xserver-1.7.7-xkb-invalid-writes.patch: fix strlen/strcpy buffer overflows
  in xkb geometry copying. (#600180)
- xserver-1.7.7-property-generation-reset.patch: reset known XI2 properties
  at the end of the server generation (#600116)
- xserver-1.7.7-device-mode-list.patch: remove the OutOfProximity bit when
  listing the device mode (#594523)

* Thu Jun 10 2010 Ben Skeggs <bskeggs@redhat.com> 1.7.7-6
- prevent unnecessary fb resize on startup with multiple heads (#602080)

* Mon Jun 07 2010 Adam Jackson <ajax@redhat.com> 1.7.7-5
- xserver-1.7.6-default-modes.patch: Add all refresh rates. (#599334)

* Fri Jun 04 2010 Dave Airlie <airlied@redhat.com> 1.7.7-4
- fix crashes when resizing redirected window pixmaps fails (#586567)

* Mon May 31 2010 Dave Airlie <airlied@redhat.com> 1.7.7-3
- update xvfb-run to latest Debian (CVE-2009-1573) (#499234)

* Wed May 26 2010 Adam Jackson <ajax@redhat.com> 1.7.7-2
- xserver 1.7.7
- xserver-1.7.{3,7}-exa-master.patch: Rebase.
- xserver-1.7.5-exa-mixed.patch: Drop, merged.
- xserver-1.7.6-{owner-events,glyph-crash,swap-function-typos,
  event-root-window-copy,cursor-pointer-reset,xkb-garbage-init,define-XINPUT,
  xinerama-cursor-screen,vfb-device-names,delivergrabbedevents-crash,
  xge-no-event,sprite-update-on-pointer-events,exa-pointer-check}.patch:
  Drop, merged.
- xserver-1.7.6-deviceevent-coordinates-xinerama.patch: Fix coordinate sign
  in Xinerama (#584927)
- xserver-1.7.6-default-modes.patch: Use DMT mode list as the default mode
  list (#585371)
...
* Fri May 07 2010 Dave Airlie <airlied@redhat.com> 1.7.6-5
- while debugging clone mode in 512023 noticed X was broken, also 568214 (#512023)
- add no connected outputs patch as well to fix disconnected operation. (#580789)

* Wed May 05 2010 Peter Hutterer <peter.hutterer@redhat.com> 1.7.6-4
- Add patches from upstream stable branch:
- xserver-1.7.6-delivergrabbedevents-crash.patch: Don't crash after two
  synchronous grabs (#588966)
- xserver-1.7.6-xge-no-event.patch: Don't register an event for XGE
  (#588975)
- xserver-1.7.6-sprite-update-on-pointer-events.patch: Update the sprite
  only for XTEST pointer events, not keyboard events (#588977)
- xserver-1.7.6-exa-pointer-check.patch: check for NULL-pointer before
  UploadToScreen (#588979)

* Mon Apr 19 2010 Peter Hutterer <peter.hutterer@redhat.com> 1.7.6-3
- Add patches from upstream stable branch, sort-of brings us in line with
  1.7.7. 
- xserver-1.7.6-glyph-crash.patch: Server crashes if all glyphs are freed
  (#583872)
- xserver-1.7.6-swap-function-typos.patch: GLX swap functions not working
  correctly (#583550)
- xserver-1.7.6-event-root-window-copy.patch: Invalid root window value for
  events in the RECORD extension.(#583551)
- xserver-1.7.6-cursor-pointer-reset.patch: Server crash when changing
  screens (#583558)
- xserver-1.7.6-xkb-garbage-init.patch: Garbage data in XKB ControlsNotify
  events. (#583557)
- xserver-1.7.6-define-XINPUT.patch: XINPUT is undefined (#583556)
- xserver-1.7.6-xinerama-cursor-screen.patch: Invalid screen check in
  Xinerama causes loops(#583871)
- xserver-1.7.6-vfb-device-names.patch: Xvfb crashes on XIQueryDevice
  (#583543)
- xserver-1.7.6-pointerkeys.patch: Pointer jumps to lower-right corner when
  clicking mouse (#583544)

* Wed Apr 14 2010 Adam Tkac <atkac redhat com> 1.7.6-2
- export more functions from Xorg to get libvnc.so module working

* Mon Mar 22 2010 Peter Hutterer <peter.hutterer@redhat.com> 1.7.6-1
- xserver 1.7.6 (#574991)
- xserver-1.7.5-record.patch: Drop.
- xserver-1.7.3-cursor-jumps.patch: Drop.
